...
# Домашняя работа к семинару 3 (файлы кортинок будут добавлены через upload на github)

## 16. Работа с удаленными репозиториями
+ ### 16.1 Клонирование репозитория на github 

    Для **копирования**, себе, интересующего нас **репозитория** на **github**, необходимо зайти в этот репозиторий на github и **произвести дествия** под названием **"fork".**

    ![Скрин кнопки fork на github](github_fork_example.jpg)

    **!!!Важно** обязательно, при клонировании репозитория через fork убрать галочку с **"copy the master branch only"!!!**

+ ### 16.2 Клонирование удаленных репозиториев с github, себе на локальный девайс

    Для **клонирования удаленного репозитория** прописываем команду **git clone код_репозитория.**

    **Код репозитория** береться, например, с сайта **"github"** в разделе **"repositories"**, **"имя_репозитория"**, вкладка **"code"** **"HTTPS".** 

    ![Скрин расположения кода на github](github_code_example.jpg)

+ ### 16.3 Отображение удаленных репозиториев

    Чтобы **просмотреть**, какие **удаленые репозитории** у нас уже настроены, пишем команду __git remote__.

  ![Скрин команды git remote в терминале](git_remote_example.jpg)

  Если **репозиторий был склонирован**, должен **отобразиться origin**, это **имя по умолчанию**, которое **git присваивает репозиторю** с которого произошло клонирование. **Если репозитореев больше чем один, git также отоброзит и их, на ряду с origin.** 

+ ### 16.4 Извлечение и сляния содержимого удаленного репозитория
    
    Для **извлечения и слияния удаленного репозитория** используются две **команды git push и git fetch.**

    **git push** - извлекает и сливает внесенные коммиты, в ветку в которой на данный момент идет работа. Это можно расмотреть, как **последовательность** двух команд **git fetch** и **git merge**. Команда **pull** автоматически сливает коммиты не давая вам просмотреть их.

    **git fetch** - извлекает, только те коммиты которых нет в текущей ветке и сохраняет их в локальном репозетории, после просмотра коммитов, иожно слить их используя **merge.**

+ ### 16.5 Передача содержимого локального репозитория в удаленный репозиторий

    Для **передачи** содержимого **локального репозитория в удаленный** репозеторий испоьзуется команда  **git push "название_репозитория".**

...
В результате выполнения данной моманды терминал выдал нам предупреждение с пометкой fatal, рекомендацией для избежания данной ошибки является выполнение команды:

    git push --set-upstream origin IlyaNovosad
